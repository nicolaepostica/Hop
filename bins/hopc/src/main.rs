...
/// Hop client.
#[derive(Debug, Parser)]
#[command(name = "hopc", version, about)]
struct Cli {
    #[command(subcommand)]
    cmd: Option<Cmd>,
    ...
    if trusted.is_empty() {
        tracing::warn!(
            "fingerprint DB is empty — cannot verify the server. \
             Add the server's fingerprint with `hopc fingerprint add`."
        );
    }
...
/// Per-OS backend selection for `hopc`.
///
...
mod backend {
    ...
    use anyhow::Result;
    use hop_client::{run, ClientConfig};
